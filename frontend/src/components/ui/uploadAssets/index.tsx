...
export default function UploadAsset({
  ...
  accept,
  setIsLoading,
}: PropsWithChildren<TUploadAssetProps>) {
  const addAssetToAvatar: ChangeEventHandler<HTMLInputElement> = async (e) => {
    if (e.target.files && e.target.files?.length) {
      ...
      const response = await Promise.allSettled<IAssets[]>(
        files.map((file) => {
  ...
  return (
    show && (
      <label className={clsx(classes.mainInfo_avatar__set, classNames)}>
        {children}
