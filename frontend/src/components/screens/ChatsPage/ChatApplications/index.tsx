...
const ChatApplications = ({ chatId }: { chatId: number }) => {
  ...
  const { data, isLoading } = useQuery({
    ...
    select: (data) => {
      ...
      const convertedData = Object.entries(groupData).map(([date, messages]) => ({
        date,
        mediases: messages?.map((message) => message.media).flat(),
      }));
  ...
  return (
    <div className={classes.applicationsBlock}>
      {isLoading && <Loader />}
      <Header activeFilter={activeFilter} setActiveFilter={setActiveFilter} />
      {data?.length ? <Applications data={data} activeFilter={activeFilter} /> : <p>Not applications</p>}
    </div>
  );
};
