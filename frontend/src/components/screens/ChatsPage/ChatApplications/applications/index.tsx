...
const Applications = ({ data, activeFilter }: { data: { date: string; mediases?: IAssets[] }[], activeFilter: string }) => {
    return <div className={classes.applicationsContent}>
        {data
            .filter((groupApplications) => activeFilter ? groupApplications.mediases?.find(media => media.mime.split('/')[0] === activeFilter) : true)
